...
public class ProprietaryApiService {
    ...
    // URL der proprietären API für Patient
    @Value("${patient.url}")
    private String patientUrl;

    // URL der proprietären API für Document
    @Value("${document.url}")
    ...
    public boolean sendPatientData(PatientDTO patientDTO) {
        try {
            ...
            logger.info("Sending request to proprietary API: " + patientUrl);

            // Sendet eine POST-Anfrage an die proprietäre API
            ResponseEntity<String> response = restTemplate.postForEntity(patientUrl, patientDTO.toJSon(), String.class);
     ...
     * Sendet Patientendaten an eine proprietäre API.
     *
     * @param documentReferenceDTO Das DTO des Document Reference
     * @return true, wenn die API-Anfrage erfolgreich war; false, wenn ein Fehler aufgetreten ist
     */

    public boolean sendDocumentReferenceData(DocumentReferenceDTO documentReferenceDTO) {
        try {

            // Loggt die URL und den Anfragekörper
            logger.info("Sending request to proprietary API: " + documentUrl);

            // Sendet eine POST-Anfrage an die proprietäre API
            ResponseEntity<String> response = restTemplate.postForEntity(documentUrl, documentReferenceDTO.toJSon(), String.class);
            ...
            // Loggt eine Ausnahme, falls eine auftritt
            logger.log(Level.SEVERE, "Exception occurred while sending document data", e);
            return false;
        }catch (JsonProcessingException e){
            // Loggt eine Ausnahme, falls eine auftritt
            logger.log(Level.SEVERE, "Exception occurred while serializing the data", e);
